...
data_1 <= bank_register(conv_integer(rt_address));
data_2 <= bank_register(conv_integer(rs_address));

write_bank_register : process (clk,RegWrite,reset)
	begin
		if reset = '1' then
			bank_register(0) <= "00000000"; -- zero
			bank_register(1) <= "00000000"; --
			bank_register(2) <= "00000000"; --
			bank_register(3) <= "00000000"; --
			bank_register(4) <= "00000000"; --
			bank_register(5) <= "00000000"; --
			bank_register(6) <= "00000000"; --
			bank_register(7) <= "00000000"; --
		elsif ((clk'event and clk ='1') and (RegWrite = '1')) then
			if (rd_address(3 downto 0) = 0) then
				bank_register(0) <= "00000000";
			else
